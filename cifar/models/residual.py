...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, stride, downsample=None, groups=1,
                in_channel=64, padding=1, norm_layer=None):
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, stride=1, downsample=None, groups=1,
                in_channel=64, dilation=1, norm_layer=None):
        super(BottleneckBlock, self).__init__()

        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        hidden_planes = int(outplanes * (in_channel / 64.)) * groups

        self.conv1 = conv1x1(inplanes, hidden_planes)
        self.bn1 = norm_layer(hidden_planes)
        self.conv2 = conv3x3(hidden_planes, hidden_planes, stride, groups, dilation)
        self.bn2 = norm_layer(hidden_planes)
        self.conv3 = conv1x1(hidden_planes, outplanes * self.expansion)
