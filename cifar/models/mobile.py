...
class SingleConvLayer(nn.Module):
    def __init__(self, inplanes, outplanes, kernel_size=1, stride=1):
        ...
        self.conv = nn.Conv2d(in_channels=inplanes, out_channels=outplanes, kernel_size=kernel_size, stride=stride)
        self.bn = nn.BatchNorm2d(outplanes)
        self.relu = nn.ReLU(inplace=True)
...
class MobileNet(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.features = nn.Sequential(
            ...
            Depthwise(1024, 1024, stride=2, padding=6),
            SingleConvLayer(1024, 1024, kernel_size=1, stride=1),
            nn.AvgPool2d(7,7),
        )
